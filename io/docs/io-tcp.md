...
通过 Linux 路由表和所需要连接的对象

```bash
$ route -n 
Destination     Gateway						Genmask
0.0.0.0					192.158.150.2     0.0.0.0

$ ping www.baidu.com
$ ping www.qq.com

# 增加路由信息
$ route add -host 192.168.88.88 gw 192.168.1.13
```

通过地址与网络路由表中的每一条中的 Genmask 地址进行按位与（&） ,结果与 Destination 进行比对。 匹配上后，就找到了下一跳地址，以上例子中“下一跳” 地址为本机网关地址 

&nbsp;

那么怎么通过下一跳 Ip 地址找到物理地址呢， 从网络层下跳到链路层找，每一层都有类似路由表的一张表
...
# DNS 会解析 域名和 ip，维护域名和 ip 的映射 （不受限）， 而 arp 解析 ip 地址和网卡硬件地址（mac 地址）的映射 （受限于同一局域网）
```

从 arp 表中就可以找到 上述网关所对应的网卡 `mac` 地址
